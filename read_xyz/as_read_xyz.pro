function read_xyz_2d3d, file, particles= prt, timesteps= tms, save= sav, _extra = _e, pN = pN
...
;		Modified by Alex Smith to be able to specify particle number
;-
...
;		stop
    ...
    ;filter by the rows that contain the particle number, then just put those in res
     pNRows = where(pres[0,*] eq pN)
     numRows = size(pNRows,/N_ELEMENTS)
     res = fltarr(6,numRows)
     res[0:2,*] = pres[1:3,pNRows] 
     n = size(res,/N_ELEMENTS)
     tms = numRows/prt
     if n eq 0 then print, "No particles of this number found :("
    endif  else begin 
    ;this is the case where the particle number isn't set so we want it to just output all of the entries
      res = fltarr(6,len)
;      stop
      res [0:2,*] = pres[1:3,*]
      endelse
    
    ;these values change depending on whether it's the PN or the non PN one
